...
export default function App() {
  ...
  // ----- PANTALLA DE LOGIN (ESTILO FINGOALS) -----
  if (!token) {
    return (
      <div className="min-h-screen bg-[#f1f5f9] flex items-center justify-center p-4 font-sans text-slate-800">
        
        <div className="bg-white shadow-[0_20px_40px_rgba(0,0,0,0.06)] border border-slate-100 p-10 rounded-[2.5rem] w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-400 text-xs font-semibold mt-1">Control de Jornadas y Horas</p>
          </div>
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1.5">Usuario</label>
              <input
                type="text"
                required
                value={usernameInput}
                onChange={(e) => setUsernameInput(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3.5 text-sm text-slate-800 font-semibold focus:outline-none focus:border-blue-600 transition-all"
            ...
            {loginRoleType === "admin" && (
              <div>
                <label className="block text-[11px] font-bold text-blue-600 uppercase mb-1.5">🔑 Token de Seguridad</label>
                <input
                  type="password"
                  required
                  value={adminTokenInput}
                  onChange={(e) => setAdminTokenInput(e.target.value)}
                  className="w-full bg-blue-50/50 border border-blue-200 rounded-2xl px-4 py-3.5 text-sm text-blue-800 font-semibold focus:outline-none focus:border-blue-600 transition-all"
            ...
            <button
              type="submit"
              className="w-full mt-4 bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-2xl shadow-xl shadow-blue-600/20 transition-all text-sm"
            >
              Iniciar Sesión
            </button>
          </form>
  ...
  // ----- PLATAFORMA PRINCIPAL (FINGOALS LAYOUT: DARK SIDEBAR + CLEAN MAIN) -----
  return (
    <div className="min-h-screen bg-[#f1f5f9] text-slate-800 flex flex-col md:flex-row font-sans">
      
      {/* Barra Lateral Oscura (Sidebar estilo Fingoals) */}
      <aside className="w-full md:w-72 bg-[#0F172A] text-white p-6 flex flex-col justify-between shrink-0 print:hidden shadow-xl">
        <div>
          ...
          <p className="text-[11px] uppercase tracking-wider text-slate-400 font-bold mb-4">Home</p>
          
          <nav className="space-y-2">
            <button 
              onClick={() => setCurrentTab("gestion")} 
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all ${currentTab === 'gestion' ? 'bg-[#2563EB] text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
              <span>📊</span> Overview / Dashboard
            </button>
            <button 
              onClick={() => setCurrentTab("estadisticas")} 
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all ${currentTab === 'estadisticas' ? 'bg-[#2563EB] text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
            ...
            {!isReadOnly && (
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all ${currentTab === 'empleados' ? 'bg-[#2563EB] text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
              >
                <span>👥</span> Contacts / Users
              </button>
            )}
          </nav>
        </div>

        {/* Tarjeta inferior tipo banner en el sidebar */}
        <div className="mt-8 bg-gradient-to-br from-blue-600 to-indigo-700 p-5 rounded-3xl text-white shadow-lg">
          ...
          <p className="font-extrabold text-sm mt-1">{isReadOnly ? "Empleado" : "Administrador"}</p>
          <button onClick={handleLogout} className="mt-4 w-full bg-white text-slate-900 font-bold py-2.5 rounded-xl text-xs hover:bg-slate-100 transition-all shadow-md">
            Cerrar Sesión
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        {/* Top Navigation Bar */}
        <header className="bg-white border-b border-slate-200/60 px-8 py-4 flex items-center justify-between sticky top-0 z-40 print:hidden shadow-sm">
          <div>
            <h1 className="text-xl font-black text-slate-900">Welcome Back!</h1>
            <p className="text-xs text-slate-400 font-medium">Current summary financial report & records</p>
          </div>

          <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-3 bg-slate-50 border border-slate-200/80 px-4 py-2 rounded-2xl">
              <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-xs">
                {userRole === 'admin' ? 'A' : 'E'}
              </div>
              <div className="text-left">
                <p className="text-xs font-bold text-slate-800">Panel Central</p>
                <p className="text-[10px] text-slate-400 capitalize">{userRole}</p>
              </div>
            </div>
            <button onClick={handleLogout} className="md:hidden bg-slate-100 text-red-500 px-3 py-1.5 rounded-xl text-xs font-bold">
              Salir
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 p-6 sm:p-10 max-w-7xl mx-auto w-full">
          {currentTab === "gestion" ? (
            <div className={`grid grid-cols-1 ${isReadOnly ? "lg:grid-cols-1" : "lg:grid-cols-3"} gap-8`}>
              ...
              {!isReadOnly && (
                <div className="bg-white border border-slate-200/80 p-8 rounded-[2rem] shadow-sm h-fit print:hidden">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="font-black text-slate-900 text-base">
                      {editingId ? "Editar Registro" : "Nuevo Registro"}
                    </h2>
                    <span className="text-blue-600 bg-blue-50 border border-blue-100 px-3.5 py-1.5 rounded-xl text-xs font-black">
                  ...
                  <form onSubmit={handleSubmitRecord} className="space-y-4">
                    <div>
                      ...
                      <input type="text" required value={workerName} onChange={(e) => setWorkerName(e.target.value)} className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-blue-600" placeholder="Nombre completo" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Fecha</label>
                      <input type="date" required value={workDate} onChange={(e) => setWorkDate(e.target.value)} className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-blue-600" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Entrada</label>
                        <input type="time" required value={entryTime} onChange={(e) => setEntryTime(e.target.value)} className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-blue-600" />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Salida</label>
                        <input type="time" required value={exitTime} onChange={(e) => setExitTime(e.target.value)} className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-blue-600" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Cédula / CC</label>
                      <input type="text" required value={costCenter} onChange={(e) => setCostCenter(e.target.value)} className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-blue-600" placeholder="Ej. 1700000000" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Descripción</label>
                      <textarea rows="2" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-blue-600 resize-none" placeholder="Opcional..."></textarea>
                    </div>
                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-blue-600/20 transition-all text-sm mt-2">
              ...
              {/* Tabla / Historial */}
              <div className={`${isReadOnly ? "lg:col-span-1" : "lg:col-span-2"} bg-white border border-slate-200/80 p-6 sm:p-8 rounded-[2rem] shadow-sm flex flex-col justify-between`}>
                <div>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 print:hidden">
                    <h2 className="font-black text-slate-900 text-lg">Financial Overview / History</h2>
                    <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                      <button onClick={handleExportExcel} className="bg-blue-50 border border-blue-100 text-blue-700 px-4 py-2 rounded-2xl text-xs font-bold transition-all">
                        Export Excel
                      </button>
                      <button onClick={handleExportPDF} className="bg-slate-100 border border-slate-200 text-slate-700 px-4 py-2 rounded-2xl text-xs font-bold transition-all">
                        Print / PDF
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 print:hidden">
                    <select value={selectedWorkerFilter} onChange={(e) => { setSelectedWorkerFilter(e.target.value); setSelectedFilterValue(""); setCurrentPage(1); }} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-700 text-xs font-bold focus:outline-none">
                      <option value="all">Todos los trabajadores</option>
                      {uniqueWorkers.map(w => <option key={w} value={w}>{w}</option>)}
                    </select>
                    <select value={filterType} onChange={(e) => { setFilterType(e.target.value); setSelectedFilterValue(""); setCurrentPage(1); }} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-700 text-xs font-bold focus:outline-none">
                      <option value="all">Filtro de Tiempo</option>
                      <option value="month">Por Mes</option>
                      <option value="week">Por Semana</option>
                    </select>
                    {filterType === "month" && (
                      <select value={selectedFilterValue} onChange={(e) => { setSelectedFilterValue(e.target.value); setCurrentPage(1); }} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-slate-700 text-xs font-bold focus:outline-none">
                        ...
                        <tbody className="divide-y divide-slate-100 text-xs font-semibold text-slate-700">
                          {currentRecords.map((rec) => (
                            <tr key={rec.id} className="hover:bg-slate-50/50 transition-colors">
                              <td className="py-4 px-4 font-black text-slate-900">{rec.worker_name || rec.trabajador}</td>
                              <td className="py-4 px-4">{rec.work_date || rec.fecha}</td>
                              <td className="py-4 px-4 text-slate-400">{rec.entry_time || rec.hora_entrada} - {rec.exit_time || rec.hora_salida}</td>
                              <td className="py-4 px-4"><span className="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-xl font-bold">{Number(rec.calculated_hours || rec.horas || 0).toFixed(1)} hrs</span></td>
                              <td className="py-4 px-4 text-slate-400">{rec.cost_center || rec.centro_costo || "-"}</td>
                              <td className="py-4 px-4 text-slate-400 max-w-[130px] truncate">{rec.description || rec.descripcion || "-"}</td>
                              {!isReadOnly && (
                                <td className="py-4 px-4 text-right space-x-2 print:hidden whitespace-nowrap">
                                  <button onClick={() => handleEdit(rec)} className="text-blue-600 hover:underline font-bold">Editar</button>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-6 print:hidden">
                    <button onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-600 disabled:opacity-40">
                      ← Anterior
                    </button>
                    <span className="text-xs text-slate-400 font-bold">
                      Página {currentPage} de {totalPages}
                    </span>
                    <button onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-600 disabled:opacity-40">
                      Siguiente →
                    </button>
                  </div>
                )}
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
              <div className="bg-white border border-slate-200/80 p-8 rounded-[2rem] shadow-sm">
                <h2 className="text-lg font-black text-slate-900 mb-1">Crear Acceso para Empleado</h2>
                <p className="text-xs text-slate-400 mb-6 font-medium">Genera una cuenta de solo visualización asociada a la cédula.</p>

                {empSuccessMsg && (
                  <div className="mb-4 p-4 bg-blue-50 border border-blue-100 rounded-2xl text-blue-600 text-xs font-bold text-center">
                    {empSuccessMsg}
                  </div>
                )}
                ...
                <form onSubmit={handleCreateEmployee} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Usuario</label>
                    <input type="text" required value={empUsername} onChange={(e) => setEmpUsername(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-800" placeholder="Ej. juan" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Contraseña</label>
                    <input type="password" required value={empPassword} onChange={(e) => setEmpPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-800" placeholder="••••••••" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1">Cédula</label>
                    <input type="text" required value={empCedula} onChange={(e) => setEmpCedula(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-800" placeholder="17000000" />
                  </div>
                  <div className="md:col-span-3">
                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-2xl transition-all text-sm shadow-lg shadow-blue-600/20">
                      ...
                      <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                        {usersList.map((u) => (
                          <tr key={u.id} className="hover:bg-slate-50">
                            <td className="py-3.5 px-3 text-slate-400">#{u.id}</td>
                            <td className="py-3.5 px-3 font-bold text-slate-900">{u.username}</td>
                            <td className="py-3.5 px-3">
                              <span className={`px-3 py-1 rounded-xl font-bold ${u.role === 'admin' ? 'bg-indigo-50 text-indigo-600' : 'bg-blue-50 text-blue-600'}`}>
                                {u.role === 'admin' ? 'Admin' : 'Empleado'}
                              </span>
                            </td>
                            <td className="py-3.5 px-3 font-mono text-slate-500">{u.cedula || "-"}</td>
            ...
            <div className="space-y-6 max-w-5xl mx-auto">
              ...
              <div className="bg-white border border-slate-200/80 p-6 rounded-[2rem] shadow-sm flex items-center justify-between">
                <div>
                  <h3 className="text-slate-900 font-black text-lg">Financial & Analytics Overview</h3>
                </div>
                <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-slate-700 text-xs font-bold focus:outline-none">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="bg-white border border-slate-200/80 p-8 rounded-[2rem] shadow-sm flex items-center justify-between">
                  <div>
                    <p className="text-slate-400 text-xs font-bold uppercase">Total Horas</p>
                    <h3 className="text-4xl font-black text-slate-900 mt-1">{totalHorasStats.toFixed(1)} <span className="text-xl text-slate-400 font-medium">hrs</span></h3>
                  </div>
                  <div className="px-3.5 py-1.5 bg-blue-50 text-blue-600 rounded-xl text-xs font-black">+18.5%</div>
                </div>
                ...
                <div className="bg-white border border-slate-200/80 p-8 rounded-[2rem] shadow-sm flex items-center justify-between">
                  ...
                  <div className="px-3.5 py-1.5 bg-blue-50 text-blue-600 rounded-xl text-xs font-black">Active</div>
                </div>
              </div>
                  ...
                  <div className="w-full h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={chartData} margin={{ top: 10, right: 30, left: -20, bottom: 5 }}>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: "#ffffff", borderColor: "#e2e8f0", borderRadius: "16px", color: "#0f172a", fontSize: "12px", fontWeight: "bold", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.1)" }} />
